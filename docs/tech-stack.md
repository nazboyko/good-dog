...
| A new server plus a stale one already on the port | the new process logs its whole startup, fails to bind, exits, and the old binary keeps answering, so you debug code that is not running | check the bind line in the log, not just the startup lines above it, and confirm which pid holds the port before believing a result |
